...
def handle_callback(request):
    ...
    try:
        ...
        if key == "ERROR":
            ...
            response.set_cookie("squad_name", squad_name)
            response.set_cookie("squad_tag", squad_tag)
...
def handle_signup(request) -> str:
    return render_template(
        "auth/signup_before_capi.html", capi_redirect_url="/auth/authorize"
    )
...
def handle_signin(request) -> str:
    if request.method == "GET":
        ...
        if user.password_hash != password_hash:
            return render_template(
                "auth/signin.html", error_message="Incorrect password, try again"
            )
        else:
            response = make_response(
                render_template(
